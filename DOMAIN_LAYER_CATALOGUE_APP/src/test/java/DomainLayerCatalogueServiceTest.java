...
public class DomainLayerCatalogueServiceTest {
    ...
    @Test
    //@Ignore
    public void testDomainCatalogueService2() {
        //Domain A objects
        final String DOMAIN_A_ID = "neutralHosting";
        DomainInterface domainAinterface = new DomainInterface("http://172.16.150.5", 8989, false, InterfaceType.HTTP);
        OsmNfvoDomainLayer osmNfvoDomainLayer = new OsmNfvoDomainLayer("osmDomainLayer", "admin", "admin", "admin");
        NHNspDomainLayer nhNspDomainLayer = new NHNspDomainLayer("NHDomainLayer", "5e6a5fc83023e56228991819", "5e6b4c413023e52b3a0f27cb");
        ArrayList<DomainLayer> listOwnedLayerDomainA = new ArrayList<DomainLayer>();
        listOwnedLayerDomainA.add(osmNfvoDomainLayer);
        listOwnedLayerDomainA.add(nhNspDomainLayer);
        Domain domainA = new Domain(DOMAIN_A_ID,"neutralHosting","neutralHosting domain","i2cat","i2cat", DomainStatus.ACTIVE, listOwnedLayerDomainA, new HashSet<>(), domainAinterface);

        //Domain A objects
        final String DOMAIN_B_ID = "neutralHosting";
        DomainInterface domainBinterface = new DomainInterface("http://172.16.150.5", 8989, false, InterfaceType.HTTP);
        osmNfvoDomainLayer = new OsmNfvoDomainLayer("osmDomainLayer2", "admin", "admin", "admin");
        nhNspDomainLayer = new NHNspDomainLayer("NHDomainLayer2", "5e6a5fc83023e56228991819", "5e6b4c7d3023e52b3a0f27cc");
        ArrayList<DomainLayer> listOwnedLayerDomainB = new ArrayList<DomainLayer>();
        listOwnedLayerDomainB.add(osmNfvoDomainLayer);
        listOwnedLayerDomainB.add(nhNspDomainLayer);
        Domain domainB = new Domain(DOMAIN_B_ID,"neutralHosting","neutralHosting domain","i2cat","i2cat", DomainStatus.ACTIVE, listOwnedLayerDomainB, new HashSet<>(), domainBinterface);

        //Domain B objects
        final String DOMAIN_C_ID = "sonata";
        DomainInterface domainCinterface = new DomainInterface("10.0.57.6", 32002, true, InterfaceType.HTTP);
        SonataNspDomainLayer sonataNspDomainLayer = new SonataNspDomainLayer("SonataDomainLayer", "5gcroco", "5gcroco");
        ArrayList<DomainLayer> listOwnedLayerDomainC = new ArrayList<DomainLayer>();
        listOwnedLayerDomainC.add(sonataNspDomainLayer);
        Domain domainC = new Domain(DOMAIN_C_ID,"sonata","sonata domain","cctc","cctc", DomainStatus.ACTIVE, listOwnedLayerDomainC, new HashSet<>(), domainCinterface);

        onBoardDomainTest(domainA);
        onBoardDomainTest(domainB);
        onBoardDomainTest(domainC);
    }
}