	...
	*/
	...
	public synchronized String onBoardVsBlueprint(OnBoardVsBlueprintRequest request)
		...
		try{
			...
			List<NST> nsts = request.getNsts();
			if(nsts!=null && !nsts.isEmpty()){
				log.debug("Onboarding NSTS");
				for(NST currentNst : nsts){
					try{
						Nsd nsd = getNstNsdFromRequest(currentNst, request);
						List<OnBoardVnfPackageRequest> vnfPackageRequests = getVnfPackageRequest(nsd, request);
						String nstInfoId = nsTemplateCatalogueService.onBoardNsTemplate(new OnBoardNsTemplateRequest(currentNst,nsd, vnfPackageRequests, null));
						vsBlueprintInfo.addNstInfoId(nstInfoId);
						//request.setNstInfoInTranslationRules(nstInfoId, currentNst.);
					}catch(AlreadyExistingEntityException e)
					{
						QueryNsTemplateResponse nstR =
								nsTemplateCatalogueService.queryNsTemplate(new GeneralizedQueryRequest(BlueprintCatalogueUtilities.buildNstInfoFilter(currentNst.getNstId(), currentNst.getNstVersion()), null));


						vsBlueprintInfo.addNstInfoId(nstR.getNsTemplateInfos().get(0).getNstInfoId());
						//request.setNstInfoInTranslationRules(nstInfoId, currentNst.);
					}
				}
			}
	...
	private Nsd getNstNsdFromRequest(NST currentNST, OnBoardVsBlueprintRequest request){
		if(request.getNsds()!=null){
			for (Nsd currentNsd: request.getNsds()){
				if (currentNsd.getNsdIdentifier().equals(currentNST.getNsdId())){
					return currentNsd;
				}
			}
			return null;
	...
	private List<OnBoardVnfPackageRequest> getVnfPackageRequest(Nsd nsd, OnBoardVsBlueprintRequest request){
		//TODO: No way of knowing which VNF Package to use without unpacking the VNF
		//For the moment the AlreaydExistingException is ignored at the NstTemplateCatalogueService

		return request.getVnfPackages();
	}
